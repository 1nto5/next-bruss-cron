...
${JSON.stringify(context, null, 2)}
      ...
      <div style="margin-top: 20px; padding: 10px; background-color: #fff3e0; border-radius: 5px;">
        <p style="margin: 0; font-size: 14px; color: #666;">
          Ten email został wygenerowany automatycznie przez system cron zadań Next-Bruss.
        </p>
      </div>
    </div>
  `;
  ...
  try {
    ...
    console.error(`Failed to send admin notification for ${jobName}:`, sendError.message);
  }
}
...
export async function executeWithErrorNotification(jobName, jobFunction) {
  try {
    ...
    await notifyAdminAboutError(jobName, error, context);
