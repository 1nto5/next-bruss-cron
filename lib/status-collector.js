...
class StatusCollector {
  ...
  /**
   * Generate summary statistics for recent executions
   * @param {number} hours - Number of hours to analyze
   */
  generateSummary(hours = 24) {
    const recentExecutions = this.getRecentExecutions(hours);
    const jobStats = {};
   ...
   * @param {number} hours - Number of hours to include in summary
   * @param {boolean} forceEvenIfEmpty - Send email even if no executions in period
   */
  async sendStatusSummary(hours = 24, forceEvenIfEmpty = true) {
    const summary = this.generateSummary(hours);

    // Skip if no executions and not forced
    if (summary.totalExecutions === 0 && !forceEvenIfEmpty) {
      console.log(`No job executions in the last ${hours} hours - skipping status summary`);
      return;
    }
    ...
    const subject = `[CRON] Status Summary - ${summary.totalExecutions} executions in ${hours}h - ${summary.generatedAt}`;
    ...
    let html = `
      <div style="font-family: Arial, sans-serif; max-width: 800px;">
        <h2 style="color: ${statusColor};">Status Summary - Last ${hours} Hours</h2>

        <div style="background-color: #f5f5f5; padding: 15px; border-radius: 5px; margin: 10px 0;">
          <p><strong>Report Period:</strong> Last ${hours} hours until ${summary.generatedAt}</p>
    ...
    if (summary.totalExecutions === 0) {
      html += `
        <div style="background-color: #fff3cd; padding: 15px; border-radius: 5px; margin: 10px 0; border-left: 4px solid #ffc107;">
          <p><strong>Note:</strong> No job executions recorded in the last ${hours} hours.</p>
      ...
      const backupExecutions = recentExecutions.filter(exec => exec.isBackup && exec.status === 'success');
      ...
      if (summary.failedExecutions === 0) {
        html += `
          <div style="background-color: #e8f5e8; padding: 15px; border-radius: 5px; margin: 10px 0; border-left: 4px solid #4caf50;">
            ...
            <p>All ${summary.totalExecutions} job executions completed successfully in the last ${hours} hours.</p>
          </div>
        `;
      }
    }
    ...
    try {
      ...
      console.log(`Status summary sent: ${summary.totalExecutions} executions, ${summary.successfulExecutions} successful, ${summary.failedExecutions} failed`);
    } catch (sendError) {
