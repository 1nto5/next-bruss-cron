...
class StatusCollector {
  ...
  async sendStatusSummary() {
    ...
    // Generate summary only for non-backup executions
    ...
    if (summary.totalExecutions === 0) {
      ...
      if (backupExecutions.length > 0) {
        ...
        Object.values(backupsByName).forEach(backup => {
          ...
          const synologyIp = result.synologyIp || '-';
          const backupName = result.backupName || backup.jobName.replace('monitor', '').replace('Backup', '');
          ...
          html += `
            <tr>
              ...
              <td style="padding: 8px; text-align: center; border: 1px solid #ccc; font-size: 12px;">${synologyIp}</td>
