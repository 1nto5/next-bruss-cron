...
async function checkPort80() {
  try {
    ...
    // Consider any response (even 500) as "port is running"
    // We only care if the service responds, not if it has errors
    return true;
  } catch (error) {
    // Only return false if connection failed (port not responding)
    return false;
  }
}
